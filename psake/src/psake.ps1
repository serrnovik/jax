...
Import-Module -Name (Join-Path -Path $scriptPath -ChildPath 'psake.psd1') -Verbose:$false -Debug:$false 1>$null 4>$null 5>$null 6>$null
if ($help) {
...
Invoke-psake $buildFile $taskList $framework $docs $parameters $properties $initialization $nologo $detailedDocs $notr -teamcity:$teamcity
