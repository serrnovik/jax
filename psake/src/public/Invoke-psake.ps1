function Invoke-psake {
    ...
    try {
        ...
        ExecuteInBuildFileScope $buildFile $MyInvocation.MyCommand.Module {
            ...
            #>>>>>>>>>>> WTF Ecraning paramerter with module
            $_module = $module
            $_initialization = $initialization
            #<<<<<<<<<<< WTF Ecraning paramerter with module

            foreach ($_key in $properties.keys) {
                if (test-path "variable:\$_key") {
                    set-item -path "variable:\$_key" -value $properties.$_key -WhatIf:$false -Confirm:$false | out-null
                }
            }
        ...
        # IMPORTANT: # Commented by @ala to catch psake errors
        #
        # $inNestedScope = ($psake.context.count -gt 1)
        # if ( $inNestedScope ) {
        #     Write-Host "Will rethrow"
        #     throw $_
        # }
        # else {
        #     Write-Host "Will not rethrow"
