        ...
        try {
            ...
            if ($global:lastexitcode -ne 0) {
                ...
                if ([string]::IsNullOrWhiteSpace($errorMessage)) {
                    ...
                    $positionString = $startPosition ? "from $($startPosition.StartLine):$($startPosition.StartColumn) to $($startPosition.EndLine):$($startPosition.EndColumn)" : "position unknown"

                    $errorMessage = "[exit code $($global:lastexitcode)] Error executing ScriptBlock at '$file' ($positionString)"
                }

                Write-Host $errorMessage -ForegroundColor Red
                if ($file -and $startPosition) {
                    $escapedFile = $file -replace '\\', '/'
                    $openLine = $startPosition.StartLine
                    $openCol = $startPosition.StartColumn
                    $pathWithPos = "$($file):${openLine}:${openCol}"
                    if (Get-Command -Name Write-Link -ErrorAction SilentlyContinue) {
                        Write-Link -TextBefore "    >> " `
                            -Text "📔 Open File (VSCode): $pathWithPos" `
                            -Href "cursor://file/$($escapedFile):${openLine}:${openCol}"

                        Write-Link -TextBefore "    >> " `
                            -Text "📄 Open File (Explorer): $pathWithPos" `
                            -Href "file:///$($escapedFile):${openLine}:${openCol}"
                    } else {
                        Write-Host "    >> Location: $pathWithPos" -ForegroundColor DarkGray
                        Write-Host "    >> cursor://file/$($escapedFile):${openLine}:${openCol}" -ForegroundColor DarkGray
                        Write-Host "    >> file:///$($escapedFile):${openLine}:${openCol}" -ForegroundColor DarkGray
                    }
                }
