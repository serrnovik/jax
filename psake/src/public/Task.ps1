function Task {
    ...
    function Wrap-ActionWithTeamCityBlock {
        param(
            [scriptblock] $block
        )

        $description = "Running psake task '$name'"
        $taskName = Escape-TeamCityValue $name
        $taskDescription = Escape-TeamCityValue $description

        $wrappedBlock = {
            Write-Host "##teamcity[blockOpened name='$taskName' description='$taskDescription']"
            ...
            try {
                ...
                $escapedMessage = $escapedMessage.Replace("[", "|[")
                $escapedMessage = $escapedMessage.Replace("]", "|]")
                $escapedMessage = $escapedMessage.Replace("`n", "|n")
                $escapedMessage = $escapedMessage.Replace("`r", "|r")

                $escapedStackTrace = $_.Exception.ToString()
                $escapedStackTrace = $escapedStackTrace.Replace("|", "||")
                $escapedStackTrace = $escapedStackTrace.Replace("'", "|'")
                $escapedStackTrace = $escapedStackTrace.Replace("[", "|[")
                $escapedStackTrace = $escapedStackTrace.Replace("]", "|]")
                $escapedStackTrace = $escapedStackTrace.Replace("`n", "|n")
                $escapedStackTrace = $escapedStackTrace.Replace("`r", "|r")
                Write-Host "##teamcity[message text='$escapedMessage' errorDetails='$escapedStackTrace' status='ERROR']"
    ...
    if ($env:TEAMCITY) {
        $action = Wrap-ActionWithTeamCityBlock $action
    }
    ...
    if ($PSCmdlet.ParameterSetName -eq 'SharedTask') {
        ...
        $hasExistingTask = $currentContext.tasks.ContainsKey($taskKey)
        $shouldReplaceDefault = $hasExistingTask -and ($taskKey -eq 'default')
        if ($hasExistingTask -and -not $shouldReplaceDefault) {
            Assert $false ($msgs.error_duplicate_task_name -f $taskKey)
        }
        if ($shouldReplaceDefault) {
            Write-Debug "Replacing default task [$taskKey)]"
        } else {
